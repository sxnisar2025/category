...
export default async function CategoryPage({ params }) {
  ...
  try {
    ...
    const categoryRes = await fetch(
      `https://www.williamjacket.com/blog/wp-json/wp/v2/categories?slug=${category}`,
      { next: { revalidate: 60 } }
    );
    ...
    if (!categoryData?.length) return notFound(); // 🟢 FIXED

    const categoryId = categoryData[0].id;

    // 🔹 Step 2: Fetch Posts of Category
    const postsRes = await fetch(
      `https://www.williamjacket.com/blog/wp-json/wp/v2/posts?categories=${categoryId}&_embed`,
      { next: { revalidate: 60 } }
    );
    ...
    if (!posts?.length) return notFound(); // 🟢 FIXED
